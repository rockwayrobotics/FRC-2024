...
public class ClimberSubsystem extends SubsystemBase {
  ...
  public ClimberSubsystem(
    int climberMotor
    /* , int topLimitSwitch, int bottomLimitSwitch */
  ) {
    ...
    m_climberMotor.setSoftLimit(
      ...
      Constants.Climber.FORWARD_MAX
    );
    m_climberMotor.setSoftLimit(
      SoftLimitDirection.kReverse,
      Constants.Climber.REVERSE_MAX
    );
    ...
    homeSensorEntry =
      dashboardTab.addPersistent("Climb Home Sensor", false).getEntry();

    climberEncoderEntry = dashboardTab.add("Climber Encoder", 0).getEntry();

    climberSpeedEntry = dashboardTab.add("Climber Speed", 0).getEntry();

    softLimitEntry = dashboardTab.add("Climber Soft Limit", false).getEntry();
  }
  ...
  public void periodic() {
    ...
    m_climberMotor.enableSoftLimit(
      SoftLimitDirection.kReverse,
      softLimitEnable
    );
    m_climberMotor.enableSoftLimit(
